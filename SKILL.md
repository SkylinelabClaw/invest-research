...
### 6. 股价走势图（K线图）自动化生成与映射逻辑
为了保证所有公司的股价走势图统一与稳定，必须严格遵循以下生成与展示逻辑：

#### 6.1 数据下载与K线图生成
1. **数据源获取**：使用 `yfinance` 或 `akshare` 等金融数据接口，先把历史交易数据批量下载到本地（涵盖美股、港股、A股等不同市场）。
2. **生成静态图**：利用下载好的数据，通过脚本自动生成标准静态的 **K线图 (Candlestick chart)**。
3. **本地化存储**：禁止使用外部 iframe 动态链接，所有生成的 K 线图必须保存为静态 `.png` 格式存放在 `charts/` 目录。

#### 6.2 页面映射展示逻辑
1. **公司详情页部署**：生成的 K 线图首先布置在**每个公司的独立详情页里**，作为财报分析与股票快照的核心配图。
2. **首页自动映射**：首页 (index.html) 的公司卡片模块，需通过底层代码/统一路径自动关联到各详情页里使用的这张股价走势图。**确保首页和详情页使用的是同一张图的映射，保持数据同源。**

#### 6.3 底层代码及文件名规范（核心防错机制！）
由于 GitHub Pages 服务器对大小写严格敏感，为彻底杜绝 404 找不到图片的问题：
1. **强制全小写**：底层代码中的公司缩写变量、HTML 文件命名、以及 `charts/` 目录下的**所有图表文件名，必须严格统一使用全小写字母**！
2. **正确示例**：`alibaba.png`、`arm.png`、`coin.png`、`google.png`、`guming.png` ✅
